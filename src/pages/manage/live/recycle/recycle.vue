<template>
	<view class="live-recycle page-full">
		<search-filter searchPlaceholder="请输入直播标题搜索" 
				...
				<list-item :item="d" :btnList='getBtnList()'
					@onBtnClick="onBtnClick" :key="i">
					 <template v-slot:content>
						<list-item-article :name="d.liveTitle" :tips='"删除人："+d.updateUserName+" 删除时间："+$formatDate(d.updateTime,"YYYY-MM-DD hh:mm:ss")' tipsColor="#999999">
...
<script>
	...
	let getListFn=liveApi.getRecycleLiveListAdmin.bind(liveApi);
	let navTitle=`直播回收站`;
	export default {
		components: {
			listItemArticle
		...
		methods: {
			...
			onBtnClick({btn:{text},item}){
				...
				if(text=="恢复"){
					this.$refs.msg.show(`<span>是否还原直播“</span><span style="color: #000;font-weight: bold">${item.liveTitle}</span><span>”？</span>`,text);
				}else if(text=="彻底删除"){
					this.$refs.msg.show(`<span>是否彻底删除直播“</span><span style="color: #000;font-weight: bold">${item.liveTitle}</span><span>”？</span>`,text);
				}
			},
			onMsgConfirm(text){
				let  r_=undefined;
				if(text=="恢复"){
					r_ = liveApi.postRecoverLiveAdmin(this.chooseItem.liveId);
				}else if(text=="彻底删除"){
					r_ = liveApi.postDeleteLiveAdmin(this.chooseItem.liveId);
				}
				if(r_){
					r_.then(data=>{
						this.getList(true);
