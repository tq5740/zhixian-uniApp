...
export default {
	...
	onLoad() {
		...
		// this.onPickerChoose({ text: this.$store.state.userInfo.org.orgName, value: this.$store.state.userInfo.org.orgId }, "org")
	...
	methods: {
		onToCrate() {
			...
			workApi.postCreateAddressAdmin(rqData).then(data => {
				this.$toast("创建成功！");
				setTimeout(() => {
