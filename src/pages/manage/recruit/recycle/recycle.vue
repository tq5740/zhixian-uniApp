				...
				<list-item :item="d" :btnList='getBtnList()'
					@onItemClick="onItemClick" @onBtnClick="onBtnClick"  :key="i">
					 <template v-slot:content>
						<list-item-article :name="d.workTitle" :tips="d | getTips" tipsColor="#999999">
...
<script>
	...
	let navTitle="职位回收站";
	let getListFn=workApi.getRecycleWorkListAdmin.bind(workApi);
	export default {
		components: {
			listItemArticle
		},
		...
		methods: {
			...
			onBtnClick({btn:{text},item}){
				...
				if(text=="彻底删除"){
					this.$refs.msg.show(`<span>您确定要彻底删除职位“</span><span style="color: #000;font-weight: bold">${item.workTitle}</span><span>”？</span>`,"delete");
				}else if(text=="恢复"){
                      this.$refs.msg.show(`<span>您确定要还原职位“</span><span style="color: #000;font-weight: bold">${item.workTitle}</span><span>”？</span>`,"recovery"); 
				}
			},
			onItemClick(item){
			},
			onMsgConfirm(flag){
				let r_=undefined;
				if("delete"==flag){
					r_=workApi.postDeleteWorkAdmin(this.chooseItem.workId)
				}else if("recovery"==flag){
					r_=workApi.postRecoverWorkAdmin(this.chooseItem.workId);
				}
				if(r_){
					r_.then(data=>{
						this.getList(true);
