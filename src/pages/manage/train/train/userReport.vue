	...
	export default {
		...
		methods: {
			...
			getList(restart = false) {
				this._getListFromServer(getListFn,restart,["trainId","query","regMode","state"]);
			},
			onItemClick(item){
				uni.navigateTo({
					url:`/pages/manage/train/train/resourceReport?trainId=${item.trainId}&userId=${item.userId}&userName=${item.realName}`
				})
			},
