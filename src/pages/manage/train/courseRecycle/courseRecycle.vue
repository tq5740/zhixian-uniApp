<template>
	<view class="course-recycle page-full">
				...
				<list-item :item="d" :btnList='getBtnList()'
					@onBtnClick="onBtnClick" :key="i">
					 <template v-slot:content>
						<list-item-article :name="d.courseTitle" :tips='"删除人："+d.updateUserName+" 删除时间："+$formatDate(d.updateTime,"YYYY-MM-DD hh:mm:ss")' tipsColor="#999999">
...
<script>
	...
	let getListFn=trainApi.getRecycleCourseListAdmin.bind(trainApi);
	let navTitle=`课程回收站`;
	export default {
		components: {
			listItemArticle
		...
		methods: {
			...
			onBtnClick({btn:{text},item}){
				...
				if(text=="恢复"){
					this.$refs.msg.show(`<span>是否还原课程“</span><span style="color: #000;font-weight: bold">${item.courseTitle}</span><span>”？</span>`,text);
				}else if(text=="彻底删除"){
					this.$refs.msg.show(`<span>是否彻底删除课程“</span><span style="color: #000;font-weight: bold">${item.courseTitle}</span><span>”？</span>`,text);
				}
			},
			onMsgConfirm(text){
				let  r_=undefined;
				if(text=="恢复"){
					r_ = trainApi.postRecoveryCourseAdmin(this.chooseItem.courseId);
				}else if(text=="彻底删除"){
					r_ = trainApi.postDeleteCourseAdmin(this.chooseItem.courseId);
				}
				if(r_){
					r_.then(data=>{
						this.getList(true);
