		...
		methods:{
			async getCategoryList(orgId){
				...
				if(!this.$isEmpty(orgId))
					rqData.orgId=orgId;
				puApi.getArticleCategoryListAdmin(rqData).then(data=>{
			...
			onBtnClick({text}){
				if(text=="编辑分类"){
					uni.navigateTo({
						url:`/pages/manage/article/category/edit?orgId=${this.chooseItem.orgId}&categoryId=${this.chooseItem.categoryId}&categoryTitle=${this.chooseItem.categoryTitle}`
					})
				}else if(text=="删除分类"){
			...
			onMsgConfirm(){
				uni.showLoading();
				puApi.postDeleteArticleCategory(this.chooseItem.categoryId).then(data=>{
						uni.hideLoading();
						this.getCategoryList(this.orgId);
					}).catch(err => {
						uni.hideLoading();
			...
			onCreateBtnClick(){
				...
				uni.navigateTo({
					url:`/pages/manage/article/category/edit?create=true&orgId=${this.orgId}`
				})
			},
		}
	}
